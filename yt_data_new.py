...
# Data Extraction
with st.sidebar:
    image1 = Image.open("New_YT_Icon.PNG")
    st.sidebar.image(image1,width=250)
    selected_option = st.sidebar.radio("**Select Functionality**", ['Extract', 'Transform', 'Analysis'])
    
if selected_option == 'Extract':
    st.header(':green[Data Extraction]')
    st.write ('(Tip:- Retrieve data from the YouTube API by specifying a channel ID and subsequently store this data in a MongoDB database.)')
    channel_id = st.text_input('**Enter channel_id**')
    Get_data = st.button('**Retrieve data and transfer it to a MongoDB**')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyC3v9vC5TMc7HhMgFSRQ8mzLWLcuRNVQtM'
...
# ========================================   /     Data Transforming from MongoDB to SQL    /   ========================================== #
...
elif selected_option == 'Transform':
    st.header(':violet[Data Transformation]')
    st.write ('''(Tip:- Transfer data from MongoDB to a SQL database to analysis.)''')
    ...
    Migrate = st.button('**Import Data into MySQL**')
...
# ====================================================   /     Channel Analysis      /   ================================================= #
    ...
    st.write ('''(Tip:- Analyse the data based on the requested questions from the below list. Select a question to get the insights)''')

    # Check available channel data
    Check_channel = st.button('**List Available Channels**')
    ...
    # Q6
        ...
        st.write('**Tip:- In November 2021, YouTube removed the public dislike count from all of its videos.**')
